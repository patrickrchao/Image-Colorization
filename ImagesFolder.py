...
class TrainFolder(datasets.ImageFolder):
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            ...
            img_transposed = img_original.transpose((1 ,2 ,0))
            img_lab = rgb2lab(img_transposed)
            print(np.mean(img_lab[:,:,0]),np.var(img_lab[:,:,0]))
            ...
            img_original = rgb2lab(img_transposed)
...
#https://github.com/shufanwu/colorNet-pytorch/blob/master/myimgfolder.py
class TrainFolder2(datasets.ImageFolder):
